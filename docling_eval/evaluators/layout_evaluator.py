...
from docling_core.types.doc.document import DoclingDocument, DocItem, DEFAULT_EXPORT_LABELS
from pydantic import BaseModel
...
class LayoutEvaluator:
    ...
    def __call__(self, ds_path: Path, split: str="test") -> DatasetLayoutEvaluation:
        ...
        true_labels, pred_labels, intersection_labels = self._find_intersecting_labels(ds)
        logging.info(f"Intersection labels: {intersection_labels}")            
            
        ground_truths = []
        predictions = []
                            
        for i, data in tqdm(enumerate(ds), desc="Layout evaluations", ncols=120, total=len(ds)):
            ...
            gts, preds = self._evaluate_layouts_in_documents(doc_id=data[BenchMarkColumns.DOC_ID],
                                                             true_doc=true_doc,
                                                             pred_doc=pred_doc,
        ...
        assert len(ground_truths)==len(predictions), "len(ground_truths)==len(predictions)"
        ...
        for key,value in result.items():
            if isinstance(value, float):
                evaluations.append(LayoutEvaluation(name=key, value=value))

        if "map_per_class" in result:
            for label_idx, class_map in enumerate(result["map_per_class"]):
                evaluations.append(LayoutEvaluation(name="Class mAP[0.5:0.95]", label=intersection_labels[label_idx].value, value=class_map))
        ...
        if "map_per_class" in result:
            for label_idx, class_map in enumerate(result["map_per_class"]):
                ...
                print(f" => {label_idx} {intersection_labels[label_idx].value}: {class_map:.3f}")

        
        return DatasetLayoutEvaluation(evaluations=evaluations, true_labels=true_labels, pred_labels=pred_labels, intersecting_labels=[_.value for _ in intersection_labels])
